...
export function writeSettings(values) {
  ...
  const next = {
    ...
    replyInstructions: String(values.replyInstructions ?? defaults.replyInstructions).trim().slice(0, 2000),
    autoCheckSeconds: boundedSeconds(values.autoCheckSeconds, defaults.autoCheckSeconds),
    autoRestartSeconds: boundedSeconds(values.autoRestartSeconds, defaults.autoRestartSeconds),
  };
...
function boundedSeconds(value, fallback) {
  ...
  return Number.isInteger(seconds) && seconds >= 10 && seconds <= 2592000 ? seconds : fallback;
}
...
export function publicSettings(values = readSettings()) {
  return {
    ...
    replyInstructions: String(values.replyInstructions || ''),
    autoCheckSeconds: values.autoCheckSeconds,
    autoRestartSeconds: values.autoRestartSeconds,
  };
}
